...
public class BuildingGrid : MonoBehaviour
{
    ...
    public bool IsAreaFree(Vector3 worldPosition, int width, int height, float rotationY)
    {
        // 根据旋转角度调整宽度和高度
        ...
        // 90度或270度旋转时，宽高互换
        if (Mathf.Abs(Mathf.DeltaAngle(rotationY, 90f)) < 1f || Mathf.Abs(Mathf.DeltaAngle(rotationY, 270f)) < 1f)
        {
            actualWidth = height;
            actualHeight = width;
        }
        
        return IsAreaFree(worldPosition, actualWidth, actualHeight);
    }
    ...
    public void OccupyArea(Vector3 worldPosition, int width, int height, float rotationY)
    {
        // 根据旋转角度调整宽度和高度
        int actualWidth = width;
        int actualHeight = height;
        ...
        OccupyArea(worldPosition, actualWidth, actualHeight);
    }
