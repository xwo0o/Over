...
public class BuildingSystem : NetworkBehaviour
{
    ...
    private float currentRotationY = 0f; // 当前旋转角度（Y轴）
    ...
    bool CanBuildHere(Vector3 position)
    {
        ...
        // 使用旋转角度检查占用区域
        if (!BuildingGrid.Instance.IsAreaFree(position, currentBuildingData.width, currentBuildingData.height, currentRotationY))
        {
            return false;
        }
    ...
    public void AttemptBuildAtPosition(Vector3 position)
    {
        ...
        {
            CmdTryBuild(currentBuildingData.buildingId, gridPosition, currentRotationY);
        }
        else
        {
        }
    ...
    // 网络命令：尝试建造
    [Command]
    void CmdTryBuild(string buildingId, Vector3 position, float rotationY)
    {
        ...
        if (!BuildingGrid.Instance.IsAreaFree(position, data.width, data.height, rotationY))
        {
            return;
        }
        ...
        // 建造建筑
        BuildStructure(buildingId, position, rotationY);
    }
    ...
    // 建造建筑
    [Server]
    void BuildStructure(string buildingId, Vector3 position, float rotationY)
    {
        BuildingData data = BuildingDataManager.Instance.GetBuildingData(buildingId);
        if (data == null)
        {
        ...
        // 异步加载并实例化建筑
        StartCoroutine(BuildStructureCoroutine(buildingId, position, rotationY, data));
    }
    
    // 建造协程
    System.Collections.IEnumerator BuildStructureCoroutine(string buildingId, Vector3 position, float rotationY, BuildingData data)
    {
        ...
        if (loadOperation.Status == AsyncOperationStatus.Succeeded)
        {
            ...
            GameObject buildingObj = Instantiate(prefab, position, Quaternion.Euler(0f, rotationY, 0f));
            ...
            // 占用网格区域（考虑旋转）
            BuildingGrid.Instance.OccupyArea(position, data.width, data.height, rotationY);
