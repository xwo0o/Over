...
public class SceneAwareAnimatorManager : MonoBehaviour
{
    ...
    public void UpdateAnimatorReference()
    {
        ...
        if (isInGameScene)
        {
            ...
            if (networkPlayer != null)
            {
                ...
                // 关键修复：配置NetworkAnimator
                if (currentAnimators.Count > 0)
                {
                    ConfigureNetworkAnimator(currentAnimators[0]);
                    OnAnimatorsInitialized?.Invoke(currentAnimators);
                }
                else
                {
    ...
    // 关键修复：防止重复配置NetworkAnimator
    private bool hasConfiguredNetworkAnimator = false;
    private Animator lastConfiguredAnimator = null;

    /// <summary>
    /// 关键修复：清理多余的NetworkAnimator组件
    /// </summary>
    private void CleanupDuplicateNetworkAnimators()
    {
        if (networkPlayer == null) return;

        NetworkAnimator[] netAnims = networkPlayer.GetComponents<NetworkAnimator>();
        if (netAnims.Length > 1)
        {
            Debug.LogWarning($"[SceneAwareAnimatorManager] 发现 {netAnims.Length} 个NetworkAnimator组件，清理多余的");
            // 保留第一个，删除其余的
            for (int i = 1; i < netAnims.Length; i++)
            {
                Destroy(netAnims[i]);
            }
        }
    ...
    /// <summary>
    /// 关键修复：配置NetworkAnimator
    /// 动态添加或更新NetworkAnimator组件，确保animator正确关联
    /// </summary>
    private void ConfigureNetworkAnimator(Animator animator)
    {
        ...
        // 关键修复：先清理多余的组件
        CleanupDuplicateNetworkAnimators();

        // 关键修复：防止重复配置同一个animator
        if (hasConfiguredNetworkAnimator && lastConfiguredAnimator == animator)
        {
            Debug.Log("[SceneAwareAnimatorManager] NetworkAnimator已经配置过，跳过");
            return;
        }

        // 查找现有的NetworkAnimator（清理后应该只有一个）
        NetworkAnimator netAnim = networkPlayer.GetComponent<NetworkAnimator>();
        
        // 如果存在但animator不匹配，删除旧的
        if (netAnim != null && netAnim.animator != animator)
        {
            Destroy(netAnim);
            netAnim = null;
            Debug.Log("[SceneAwareAnimatorManager] 删除旧的NetworkAnimator组件");
        }
        ...
        if (netAnim == null)
        {
            ...
            Debug.Log("[SceneAwareAnimatorManager] 动态添加NetworkAnimator组件");
        }
        ...
        // 记录已配置状态
        hasConfiguredNetworkAnimator = true;
        lastConfiguredAnimator = animator;
        
        Debug.Log($"[SceneAwareAnimatorManager] NetworkAnimator已配置 - Animator={animator.name}");
    }
