...
public class NetworkModeSelectionUIController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            networkModePanel.SetActive(true); // 默认显示网络模式选择面板
            ...
            Debug.Log("[NetworkModeSelectionUIController] 网络模式选择面板已显示");
        }
    ...
    void OnHostModeButtonClicked()
    {
        ...
        // 角色选择已迁移到GameScene，这里只保存网络模式
        // 默认使用角色"01"，在GameScene中可以切换
        string characterId = PlayerSelectionData.SelectedCharacterId;
        if (string.IsNullOrEmpty(characterId))
        {
            characterId = "01"; // 默认角色
            Debug.Log("[NetworkModeSelectionUIController] 使用默认角色: 01");
        }
