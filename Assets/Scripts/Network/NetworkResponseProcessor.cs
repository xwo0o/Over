...
namespace NetworkCore
{
    public class NetworkResponseProcessor : NetworkBehaviour
    {
        ...
        private NetworkResponse ProcessPlayerAttack(NetworkRequest request)
        {
            ...
            // 使用 WeaponStateManager 处理攻击
            WeaponStateManager weaponStateManager = player.GetComponent<WeaponStateManager>();
            if (weaponStateManager != null)
            {
                // 注意：RequestAttack 是公共接口，内部会调用 Command
                // 但 NetworkResponseProcessor 在服务器运行，需要直接调用服务器方法
                // 由于我们在服务器端，直接调用 ServerStartAttack
                // 但 ServerStartAttack 是 private，需要改为 internal 或添加公共服务器接口
                // 暂时使用反射或直接调用 Command（服务器调用 Command 会立即执行）
                
                // 服务器端直接调用攻击逻辑
                // 注意：这里假设 NetworkResponseProcessor 在服务器运行
                // 我们需要一个可以在服务器调用的公共方法
                weaponStateManager.RequestAttack();
                return new NetworkResponse(request.requestId, NetworkResponseCode.Success, "攻击指令已处理");
            }
            else
            {
                return new NetworkResponse(request.requestId, NetworkResponseCode.NotFound, "未找到武器状态管理器");
            }
        }
