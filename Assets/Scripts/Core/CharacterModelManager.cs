...
public class CharacterModelManager : NetworkBehaviour
{
    ...
    public override void OnStartClient()
    {
        ...
        // 关键修复：所有客户端都使用MonitorCharacterIdChanges来监听角色ID变化
        // 这样可以确保当主机切换角色时，所有客户端都能收到通知并更新模型
        if (isLocalPlayer)
        {
        ...
        {
            Debug.Log($"[CharacterModelManager] 远程玩家，开始监听角色ID变化");
            StartCoroutine(MonitorCharacterIdChanges());
        }
    }
    ...
    private void InstantiateModel(GameObject modelPrefab)
    {
        ...
        Debug.Log($"[CharacterModelManager] 角色模型实例化成功: ID {currentCharacterData?.id}");
    ...
    {
        Debug.Log($"[CharacterModelManager] 开始切换角色模型: {newCharacterId}");
        ...
        if (currentModel != null)
        {
            Destroy(currentModel);
            currentModel = null;
            Debug.Log("[CharacterModelManager] 旧模型已销毁");
        }
