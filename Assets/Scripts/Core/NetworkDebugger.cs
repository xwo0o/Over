...
public class NetworkDebugger : MonoBehaviour
{
    ...
    void CheckCharacterDatabase()
    {
        ...
        {
            Debug.Log($"  - ID: {character.id}, 生命值: {character.health}, 速度: {character.speed}");
        }
        
        CharacterData firstChar = CharacterDatabase.Instance.GetCharacter("01");
        if (firstChar != null)
        {
            Debug.Log("[角色数据库] 角色01数据验证通过");
        }
        else
        {
            Debug.LogError("[角色数据库] 角色01数据未找到！");
        }
        Debug.Log("=================================");
    }
    ...
    {
        Debug.Log($"[角色数据] 已应用角色数据到 {stats.characterId}");
