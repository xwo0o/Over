...
public class EnemyHealthManager : NetworkBehaviour, IDamageable
{
    ...
    public void TakeDamage(float damage, uint attackerNetId)
    {
        try
        {
            ...
            Debug.Log($"[EnemyHealthManager] 敌人受伤 - 类型: {enemyType}, 攻击者: {attackerNetId}, 当前血量: {currentHealth}, 伤害: {amount}, 新血量: {newHealth}");
        ...
        {
            Debug.LogError("[EnemyHealthManager] TakeDamage方法异常: " + ex.Message);
        }
    }
